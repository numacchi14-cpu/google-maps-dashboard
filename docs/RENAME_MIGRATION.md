...
| 次に行う作業 | ①コード・設定・文書内に旧名称`Spotlog`・旧URL`spotlog.num-ish.com`の不要な参照が残っていないか調査（1.1.10節、M6未実施分）、②Google OAuth同意画面のアプリ名・ホームページURL・プライバシーポリシーURL・利用規約URLを新ブランド・新URLへ更新するかの判断・実施（M5残項目）、③公式サイト側のCloudflare Pages本番反映状況をユーザーに確認、④本書の今回の更新（M6進捗記録）をコミット・pushし、SPEC.mdとの整合を確認 |
| 作業停止理由 | 旧URLのDNS・OAuth生成元は削除済みのため、その点での停止要因はなし。公式サイト側のCloudflare Pages本番反映状況は未確認のまま（前回記録したCloudflare API障害が復旧しているかは未確認。下記「現在保留中の外部要因」参照） |
| 削除・変更してはいけない項目 | 下記「削除・変更してはいけない項目」参照（2026-08-01付記あり） |
| 最終更新日 | 2026-08-01 |
| 最終更新時のコミットSHA | `c051f77`（`docs: record Cloudflare Pages outage blocking official site rollout`）。本書の今回の更新（M6進捗記録：旧URL削除・新URL再確認）はこの時点でまだ未コミット |
...
- **チェックボックス**：
  ...
  - [ ] コード、設定、文書内に不要な`Spotlog`表記が残っていないか調査（1.1.10節）※未実施
  ...
  - [x] 移行完了を本書に記録（本更新）※SPEC.mdへの反映は未実施（下記「注意事項」参照）
- **注意事項**：
  - 旧URLの削除は元に戻しにくい操作のため、実施前に必ずユーザーの明示的な承認を得ること
    （今回はユーザー自身が実施・報告した内容を記録）
  - SPEC.md 1.1節（1.1.7〜1.1.10）・5節にはまだ「削除済み」の状態が反映されていない。
    次回以降のセッションでSPEC.md側も更新すること
  - 上記チェックボックスのうち、コード・設定・文書内の旧名称`Spotlog`表記の残存調査、
    およびOAuth同意画面のアプリ名・URL群の更新（M5残項目）はまだ未実施
