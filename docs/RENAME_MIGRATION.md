...
| 項目 | 内容 |
|---|---|
| 現在フェーズ | M0 完了 → M1 未着手（着手待ち） |
| 完了済みフェーズ | M0（現在地の確定） |
| 次に行う作業 | M1「変更前の基準点作成」の着手（本書・SPEC.mdのコミット・pushを含む） |
| 作業停止理由 | ユーザー指示により、本格的な移行作業は数日後に開始予定。今回は文書化のみを実施し、意図的にここで停止している（ブロッカーやエラーによる停止ではない） |
| 削除・変更してはいけない項目 | 下記「削除・変更してはいけない項目」参照 |
| 最終更新日 | 2026-07-30 |
| 最終更新時のコミットSHA | `a5e648d636d63be57708b5926b959a5e6d5445d7`（この時点ではSPEC.md更新のみがコミット済み。本書自体は未コミット） |
| 次回作業開始時の確認事項 | 下記「次回作業開始時の確認事項」参照 |

### 削除・変更してはいけない項目（現時点・全フェーズ共通）

- `spotlog.num-ish.com`：新URLでの動作確認（M4完了）まで削除・転送停止しない（SPEC.md 1.1.8）
- Google Drive上の保存ファイル名 `g-map-dashboard-backup.json`：名称変更を理由に変更しない（SPEC.md 1.1.9）
...
- 現行のOAuthクライアント設定（承認済みJavaScript生成元・リダイレクトURI・ホームページURL・プライバシーポリシーURL `https://spotlog.num-ish.com/privacy.html`）：M4で新URLを「追加」するまでは既存設定を残す
- エクスポート・インポート形式（CSV/JSON）、ユーザーIDとの紐付け：変更しない

### 次回作業開始時の確認事項

1. `git log -3` / `git status` で、このリポジトリの最新コミットと作業ツリーの状態を確認する
...
- **チェックボックス**：
  - [ ] 本書（`docs/RENAME_MIGRATION.md`）とSPEC.mdの更新をコミット・push
  - [ ] `git status` でworking tree cleanを確認
  - [ ] OAuthクライアントの現在設定（承認済みJavaScript生成元・リダイレクトURI・
        ホームページURL・プライバシーポリシーURL・利用規約URL）をCloud Consoleで
        確認し本書に追記
  - [ ] GitHub Pagesの現在のカスタムドメイン設定（`CNAME`の中身）を確認・記録
  - [ ] Cloudflareの現在のDNSレコードを確認・記録
...
- **作業内容**：SPEC.md 1.1.7節（変更対象候補）・1.1.9節（データ互換性）を参照し、
  実際の書き換え箇所の洗い出しと影響確認を行う（本番反映はM5で行う。このフェーズは
  「準備」＝洗い出しと計画までとする）
...
- **完了条件**：SPEC.md 1.1.7節の変更対象候補について、それぞれ「変更する／しない」の
  判断と変更内容の草案が一覧化されている
- **チェックボックス**：
  - [ ] `index.html`内の表示名称・title・meta・OGP箇所の洗い出し
  - [ ] `manifest.json`の`name`/`short_name`の変更要否確認
  - [ ] アイコン・ロゴ内の文字（該当あれば）の確認
  - [ ] `app.js`/`sw.js`/`index.html`内の`spotlog.num-ish.com`ハードコード箇所の洗い出し
  - [ ] SPEC.md 1.1.9節の「安易に変更しない」項目に該当するものが無変更であることの再確認
  - [ ] README・テスト内の旧名称箇所の洗い出し
- **注意事項**：
  - この段階では実際のコード変更・設定変更は行わない（洗い出しと計画のみ）
  - 1.1.9節に該当する項目を変更する必要が生じた場合は、先に「既存データの検出／
    新形式への移行／移行失敗時の復旧／後方互換性／バックアップ方法／テスト項目」の
    設計を用意すること
- **次に作業するリポジトリ**：他方のリポジトリ（M3の着手をユーザー経由で依頼）。
  並行してこのリポジトリはM4の準備を進めてよい
...
- **チェックボックス**：
  - [ ] CloudflareのDNSレコード追加（新サブドメイン）
  - [ ] GitHub Pagesカスタムドメイン設定の追加/更新
  - [ ] OAuthクライアントの承認済みJavaScript生成元に新URLを追加
  - [ ] 新URLでアプリが正常に表示される（1.1.10節）
  - [ ] PCとスマートフォンで主要画面が正常に表示される（1.1.10節）
  - [ ] Googleログインが正常に動作する（1.1.10節）
  - [ ] Google Driveへの保存・読込みが正常に動作する（1.1.10節）
  - [ ] Google Takeoutデータを取り込める（1.1.10節）
  - [ ] 手入力データを登録・編集できる（1.1.10節）
  - [ ] 行った場所・行きたい場所を管理できる（1.1.10節）
  - [ ] Googleカテゴリーと独自カテゴリーを管理できる（1.1.10節）
  - [ ] 既存データを引き続き読み込める（1.1.10節）
  - [ ] PWAとしてインストールできる（1.1.10節）
  - [ ] Service Workerやキャッシュに旧名称・旧URLの問題がない（1.1.10節）
- **注意事項**：
...
- **次に作業するリポジトリ**：両リポジトリ（M5へ、ブランドの一括切替）
...
- **完了条件**：アプリ・OAuth同意画面の名称・URL表記が新ブランドに統一されており、
  ...
  必要な場合は申請済みである
- **チェックボックス**：
  - [ ] SPEC.md 1.1.7節の変更対象候補をすべて`スポット帖`表記に更新（このリポジトリ）
  - [ ] `CNAME`ファイルを新URLに更新
